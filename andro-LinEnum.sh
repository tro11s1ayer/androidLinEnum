...
release=`cat /vendor/etc/*-release 2>/dev/null`
if [ "$release" ]; then
  echo -e "\e[00;31m[-] Specific release information:\e[00m\n$release" 
  echo -e "\n" 
fi
release=`cat /system/etc/*-release 2>/dev/null`
if [ "$release" ]; then
  echo -e "\e[00;31m[-] Specific release information:\e[00m\n$release" 
  echo -e "\n" 
fi
...
grpinfo=`for i in $(cut -d":" -f1 /vendor/etc/passwd 2>/dev/null);do id $i;done 2>/dev/null`
if [ "$grpinfo" ]; then
...
#checks to see if any hashes are stored in /vendor/etc/passwd (depreciated  *nix storage method)
hashesinpasswd=`grep -v '^[^:]*:[x]' /vendor/etc/passwd 2>/dev/null`
if [ "$hashesinpasswd" ]; then
  echo -e "\e[00;33m[+] It looks like we have password hashes in /vendor/etc/passwd!\e[00m\n$hashesinpasswd" 
  echo -e "\n"
fi

#contents of /vendor/etc/passwd
readpasswd=`cat /vendor/etc/passwd 2>/dev/null`
if [ "$readpasswd" ]; then
  echo -e "\e[00;31m[-] Contents of /vendor/etc/passwd:\e[00m\n$readpasswd" 
  echo -e "\n"
fi

if [ "$export" ] && [ "$readpasswd" ]; then
  mkdir $format/etc-export/ 2>/dev/null
  cp /vendor/etc/passwd $format/etc-export/passwd 2>/dev/null
fi

#checks to see if the shadow file can be read
readshadow=`cat /vendor/etc/shadow 2>/dev/null`
if [ "$readshadow" ]; then
...
if [ "$export" ] && [ "$readshadow" ]; then
  mkdir $format/etc-export/ 2>/dev/null
  cp /vendor/etc/shadow $format/etc-export/shadow 2>/dev/null
fi
...
#checks to see if /vendor/etc/master.passwd can be read - BSD 'shadow' variant
readmasterpasswd=`cat /vendor/etc/master.passwd 2>/dev/null`
if [ "$readmasterpasswd" ]; then
  echo -e "\e[00;33m[+] We can read the master.passwd file!\e[00m\n$readmasterpasswd" 
  echo -e "\n"
fi
...
if [ "$export" ] && [ "$readmasterpasswd" ]; then
  mkdir $format/etc-export/ 2>/dev/null
  cp /vendor/etc/master.passwd $format/etc-export/master.passwd 2>/dev/null
fi

#all root accounts (uid 0)
superman=`grep -v -E "^#" /vendor/etc/passwd 2>/dev/null| awk -F: '$3 == 0 { print $1}' 2>/dev/null`
if [ "$superman" ]; then
...
#pull out vital sudoers info
sudoers=`grep -v -e '^$' /vendor/etc/sudoers 2>/dev/null |grep -v "#" 2>/dev/null`
if [ "$sudoers" ]; then
  echo -e "\e[00;31m[-] Sudoers configuration (condensed):\e[00m$sudoers"
  echo -e "\n"
fi
...
if [ "$export" ] && [ "$sudoers" ]; then
  mkdir $format/etc-export/ 2>/dev/null
  cp /vendor/etc/sudoers $format/etc-export/sudoers 2>/dev/null
fi
...
#pull out account names from /vendor/etc/passwd and see if any users have associated cronjobs (priv command)
cronother=`cut -d ":" -f 1 /vendor/etc/passwd | xargs -n1 crontab -l -u 2>/dev/null`
if [ "$cronother" ]; then
...
initdread=`ls -la /etc/init 2>/dev/null`
if [ "$initdread" ]; then
  echo -e "\e[00;31m[-] /etc/init/ binary permissions:\e[00m\n$initdread" 
  echo -e "\n"
fi


initdread=`ls -la /vendor/etc/init 2>/dev/null`
if [ "$initdread" ]; then
  echo -e "\e[00;31m[-] /vendor/etc/init/ binary permissions:\e[00m\n$initdread" 
  echo -e "\n"
fi


#init.d files NOT belonging to root!
initdperms=`find /etc/init/ \! -uid 0 -type f 2>/dev/null |xargs -r ls -la 2>/dev/null`
if [ "$initdperms" ]; then
  echo -e "\e[00;31m[-] /etc/init/ files not belonging to root:\e[00m\n$initdperms" 
  echo -e "\n"
fi

rcdread=`ls -la /etc/rc.d/init 2>/dev/null`
if [ "$rcdread" ]; then
  echo -e "\e[00;31m[-] /etc/init binary permissions:\e[00m\n$rcdread" 
  echo -e "\n"
fi
...
#init.d files NOT belonging to root!
rcdperms=`find /etc/init \! -uid 0 -type f 2>/dev/null |xargs -r ls -la 2>/dev/null`
if [ "$rcdperms" ]; then
  echo -e "\e[00;31m[-] /etc/rc.d/init.d files not belonging to root:\e[00m\n$rcdperms" 
  echo -e "\n"
fi

usrrcdread=`ls -la /system/usr/local/etc/init 2>/dev/null`
if [ "$usrrcdread" ]; then
  echo -e "\e[00;31m[-] /system/usr/local/etc/ binary permissions:\e[00m\n$usrrcdread" 
  echo -e "\n"
fi

#rc.d files NOT belonging to root!
usrrcdperms=`find /system/usr/local/etc/init \! -uid 0 -type f 2>/dev/null |xargs -r ls -la 2>/dev/null`
if [ "$usrrcdperms" ]; then
  echo -e "\e[00;31m[-] /system/usr/local/etc/init files not belonging to root:\e[00m\n$usrrcdperms" 
  echo -e "\n"
fi
...
initread=`ls -la /vendor/etc/init/ 2>/dev/null`
if [ "$initread" ]; then
  echo -e "\e[00;31m[-] /vendor/etc/init/ config file permissions:\e[00m\n$initread"
  echo -e "\n"
fi
...
# upstart scripts not belonging to root
initperms=`find /vendor/etc/init \! -uid 0 -type f 2>/dev/null |xargs -r ls -la 2>/dev/null`
if [ "$initperms" ]; then
   echo -e "\e[00;31m[-] /vendor/etc/init/ config files not belonging to root:\e[00m\n$initperms"
   echo -e "\n"
fi

systemdread=`ls -lthR /vendor/lib/systemd/ 2>/dev/null`
if [ "$systemdread" ]; then
  echo -e "\e[00;31m[-] /vendor/lib/systemd/* config file permissions:\e[00m\n$systemdread"
  echo -e "\n"
fi

systemdread=`ls -lthR /system/lib/systemd/ 2>/dev/null`
if [ "$systemdread" ]; then
  echo -e "\e[00;31m[-] /system/lib/systemd/* config file permissions:\e[00m\n$systemdread"
  echo -e "\n"
fi

# systemd files not belonging to root
systemdperms=`find /vendor/lib/systemd/ \! -uid 0 -type f 2>/dev/null |xargs -r ls -la 2>/dev/null`
if [ "$systemdperms" ]; then
   echo -e "\e[00;33m[+] /vendor/lib/systemd/* config files not belonging to root:\e[00m\n$systemdperms"
   echo -e "\n"
fi

# systemd files not belonging to root
systemdperms=`find /system/lib/systemd/ \! -uid 0 -type f 2>/dev/null |xargs -r ls -la 2>/dev/null`
if [ "$systemdperms" ]; then
   echo -e "\e[00;33m[+] /system/lib/systemd/* config files not belonging to root:\e[00m\n$systemdperms"
   echo -e "\n"
fi
}
...
if [ "$thorough" = "1" ]; then
  apachehomedirs=`ls -alhR /var/www/ 2>/dev/null; ls -alhR /srv/www/htdocs/ 2>/dev/null; ls -alhR /system/usr/local/www/apache2/data/ 2>/dev/null; ls -alhR /opt/lampp/htdocs/ 2>/dev/null`
...
echo -e "\e[00;31m[-] Can we read/write sensitive files:\e[00m" ; ls -la /vendor/etc/passwd 2>/dev/null ; ls -la /etc/group 2>/dev/null ; ls -la /etc/profile 2>/dev/null; ls -la /vendor/etc/shadow 2>/dev/null ; ls -la /etc/master.passwd 2>/dev/null 
echo -e "\n" 
...
bsdusrplan=`find /system/usr/home -iname *.plan -exec ls -la {} \; -exec cat {} 2>/dev/null \;`
if [ "$bsdusrplan" ]; then
  echo -e "\e[00;31m[-] Plan file permissions and contents:\e[00m\n$bsdusrplan" 
  echo -e "\n"
fi
...
bsdrhostsusr=`find /system/usr/home -iname *.rhosts -exec ls -la {} 2>/dev/null \; -exec cat {} 2>/dev/null \;`
...
if [ "$thorough" = "1" ]; then
  #phackt
  #displaying /vendor/vendor/etc/fstab.mt6739.mt6739
  fstab=`cat /vendor/etc/fstab.mt6739 2>/dev/null`
  if [ "$fstab" ]; then
...
#looking for credentials in /vendor/etc/fstab.mt6739
fstab=`grep username /vendor/etc/fstab.mt6739 2>/dev/null |awk '{sub(/.*\username=/,"");sub(/\,.*/,"")}1' 2>/dev/null| xargs -r echo username: 2>/dev/null; grep password /vendor/etc/fstab.mt6739 2>/dev/null |awk '{sub(/.*\password=/,"");sub(/\,.*/,"")}1' 2>/dev/null| xargs -r echo password: 2>/dev/null; grep domain /vendor/etc/fstab.mt6739 2>/dev/null |awk '{sub(/.*\domain=/,"");sub(/\,.*/,"")}1' 2>/dev/null| xargs -r echo domain: 2>/dev/null`
if [ "$fstab" ]; then
  echo -e "\e[00;33m[+] Looks like there are credentials in /vendor/etc/fstab.mt6739!\e[00m\n$fstab"
  echo -e "\n"
fi

if [ "$export" ] && [ "$fstab" ]; then
  mkdir $format/etc-exports/ 2>/dev/null
  cp /vendor/etc/fstab.mt6739 $format/etc-exports/fstab done 2>/dev/null
fi

fstabcred=`grep cred /vendor/etc/fstab.mt6739 2>/dev/null |awk '{sub(/.*\credentials=/,"");sub(/\,.*/,"")}1' 2>/dev/null | xargs -I{} sh -c 'ls -la {}; cat {}' 2>/dev/null`
if [ "$fstabcred" ]; then
    echo -e "\e[00;33m[+] /vendor/etc/fstab.mt6739 contains a credentials file!\e[00m\n$fstabcred" 
    echo -e "\n"
fi
